...
pub use super::_entities::subscriptions::{ActiveModel, Column, Entity, Model};
...
impl Model {
    /// Finds all subscriptions for an org.
    ...
    /// Finds active subscriptions for an org.
    pub async fn find_active_by_org(db: &DatabaseConnection, org_id: i32) -> Vec<Self> {
        Entity::find()
            .filter(Column::OrgId.eq(org_id))
    ...
    /// Finds a subscription by pid, scoped to an org.
