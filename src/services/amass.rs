...
/// Structured per-host record extracted from subfinder JSONL.
///
/// `addresses` is intentionally kept (always empty) to preserve the on-disk
/// shape of older `amass`-era job runs — the topology view in
/// `controllers::home` reads it to mark a subdomain as resolved. With
/// subfinder, resolution status comes from `asm_scan` (DNS lookups happen
/// there).
